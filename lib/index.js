...
exports.alloc = function () {
    var buf = Buffer.alloc.apply(null, arguments);
...
exports.compare = function (buf1, buf2) {
    if (buf1 instanceof Buffer) {
        if (buf2 instanceof Buffer) return Buffer.compare(buf1, buf2);else if (buf2 instanceof BufferPlus) return Buffer.compare(buf1, buf2.toBuffer());else throw new TypeError('Arguments must be Buffer or BufferPlus');
    } else if (buf1 instanceof BufferPlus) {
        if (buf2 instanceof Buffer) return Buffer.compare(buf1.toBuffer(), buf2);else if (buf2 instanceof BufferPlus) return Buffer.compare(buf1.toBuffer(), buf2.toBuffer());else throw new TypeError('Arguments must be Buffer or BufferPlus');
    } else {
...
exports.concat = function (list, length) {
    ...
    if (list.length === 0) return BufferPlus();
    ...
    } else throw new TypeError('"list" argument must be an Array of Buffers or BufferPlus');
...
exports.create = function (arg) {
    ...
    if (typeof arg === 'number' || arg instanceof Buffer || arg instanceof BufferPlus) return new BufferPlus(arg);else if (arg === undefined) return new BufferPlus(64);else throw TypeError('argument should be Buffer, BufferPlus or number of size');
};

exports.from = function (value, encodingOrOffset, length) {
    var buf = void 0;
    if (value instanceof BufferPlus || value instanceof Buffer) buf = value;else buf = Buffer.from.apply(null, arguments);
...
exports.addCustomType = function (name, readFunction, writeFunction, sizeFunction) {
    ...
    if (typeof name !== 'string' || !validNameRegexp.test(name)) throw new TypeError('name must be a valid function name');
