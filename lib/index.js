...
exports.from = function () {
    var buf = void 0;
    if (arguments.length === 1 && arguments[0] instanceof BufferPlus) buf = arguments[0];else buf = Buffer.from.apply(null, arguments);
    return new BufferPlus(buf);
};

exports.addCustomType = function (name, readFunction, writeFunction, sizeFunction) {
    var validNameRegexp = /^[a-z0-9]+$/i;
    if (typeof name !== 'string' || !validNameRegexp.test(name)) throw new TypeError('name must be a valid string which contains only alphanumeric characters');
    ...
    BufferPlus._registerCustomType(name, BufferPlus.prototype['read' + name], BufferPlus.prototype['write' + name], BufferPlus.prototype['byteLength' + name]);
};
