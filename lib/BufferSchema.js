...
var helper = {};
var ObjectRequiredFields = ['type', 'properties', 'order'];
...
class BufferSchema {
  ...
  addField(key, dataType) {
    ...
    if (dataType instanceof BufferSchema) {
      var schema = dataType;
      var funcName = "Schema".concat(schema.name);
      this._decodeInnerFuncs[schema.name] = schema.getDecodeNameFuncStr("_read".concat(funcName));
      this._encodeInnerFuncs[schema.name] = schema.getEncodeNameFuncStr("_write".concat(funcName));
      this._byteLengthInnerFuncs[schema.name] = schema.getByteLengthNameFuncStr("_byteLength".concat(funcName));
      funcStr = {
  ...
  addArrayField(key, dataType) {
    ...
    if (dataType instanceof BufferSchema) {
      var schema = dataType;
      var schemaName = "Schema".concat(schema.name);
      this._decodeInnerFuncs[schema.name] = schema.getDecodeNameFuncStr("_read".concat(schemaName));
      this._encodeInnerFuncs[schema.name] = schema.getEncodeNameFuncStr("_write".concat(schemaName));
      this._byteLengthInnerFuncs[schema.name] = schema.getByteLengthNameFuncStr("_byteLength".concat(schemaName));
      funcStr = {
  ...
  _getSchemaInstance(name) {
    if (BufferPlus.hasSchema(name)) {
      ...
      var schema = new BufferSchema(name);
  ...
  _compileSchemaArray(schemaName, name, schema) {
    ...
    var typeLowerCase = schema.type.toLowerCase();
    debug("_compileSchemaArray schemaName: ".concat(schemaName, ", schema.name: ").concat(schema.name, ", name: ").concat(name, " type: ").concat(typeLowerCase));

    if (typeLowerCase === 'object') {
      this._compileSchemaObject(schemaName, schema);
    } else if (typeLowerCase === 'schema') {
      var nestSchema = BufferPlus.getSchema(schema.name);
  ...
  _compileSchemaObject(schemaName, schema) {
    ...
    var properties = Object.getOwnPropertyNames(schema.properties);
    ...
    schema.order.forEach(function (field) {
    ...
    schema.order.forEach(function (name) {
      ...
      debug("_compileSchemaObject schemaName: ".concat(schemaName, ", schema.name: ").concat(schema.name, ", name: ").concat(name, " type: ").concat(typeLowerCase));
  ...
  buildOnce() {
    var needBuildSchema = this._schema && !this._buildSchema;

    if (!this._buildOnce || needBuildSchema) {
      if (needBuildSchema) {
        debug('_schema:', this._schema);

        var schemaType = this._schema.type.toLowerCase();
