...
class BufferSchema {
  ...
  addField(key, dataType) {
    key = key.trim();
    var funcStr;
    var readObjStr = "data['".concat(key, "']");
    var writeObjStr = "json['".concat(key, "']");
  ...
  addArrayField(key, dataType) {
    key = key.trim();
    var readItem = "data['".concat(key, "']");

    var writeItem = this._addRefVal("json['".concat(key, "']"));
...
function _genWriteStr(funcName, valueStr) {
  return "helper.offset =buffer.write".concat(funcName, "(").concat(valueStr, ", helper.offset, true);");
}
...
function getBuiltinSizeString(dataType, valueStr) {
  ...
  if (typeof funcMap.size === 'number') return "byteCount += ".concat(funcMap.size, ";");
...
function getDataTypeFunctionString(type, readObjStr, writeObjStr) {
  ...
  if (READ_BUILTIN_TYPES.hasOwnProperty(typeLowerCase)) {
    return {
      ...
      return {
        read: "".concat(readObjStr, " = buffer.read").concat(dataType, "();"),
        write: "buffer.write".concat(dataType, "(").concat(writeObjStr, ");"),
        size: getBuiltinSizeString(dataType, writeObjStr)
      };
    } else {
