...
class BufferSchema {
    ...
    setEncoding(encoding) {
        if (!BufferPlus.isEncoding(encoding)) throw new TypeError('encoding must be a valid string encoding');
    ...
    addField(key, dataType) {
        key = key.trim();
        var funcStr = void 0;
        var readObjStr = 'data[\'' + key + '\']';
        var writeObjStr = 'json[\'' + key + '\']';
        if (dataType instanceof BufferSchema) {
            var schema = dataType;
            var funcName = 'Schema' + schema.name;
    ...
    addArrayField(key, dataType) {
        key = key.trim();
        var readItem = 'data[\'' + key + '\']';
        var writeItem = this._addRefVal('json[\'' + key + '\']');

        var arrayLenVal = getArrayValStr();
        var arrayIter = getIterStr();
        var readObjStr = readItem + '[' + arrayIter + ']';
        var writeObjStr = writeItem + '[' + arrayIter + ']';

        var arrayLenVal2 = getArrayValStr();
        var arrayIter2 = getIterStr();

        var writeObjStr2 = writeItem + '[' + arrayIter2 + ']';
    ...
    _buildDecodeFuncStr(inner) {
        var rootPrefix = inner ? [] : ['var buffer = rBuffer._buf;', 'helper.offset = 0;'];
        var decodePrefixs = ['var strEnc = \'' + this._encoding + '\';', 'var data = {};'];
        var decodeSuffixs = [inner ? '' : 'rBuffer._forceMoveTo(helper.offset);', 'return data;'];
    ...
    _buildEncodeFuncStr(inner) {
        var encodePrefixs = [inner ? [] : 'var strEnc = \'' + this._encoding + '\';'];
        ...
        }return this._compactFuncStr(encodePrefixs.concat(this._refCtx, inner ? [] : 'helper.offset = 0;', inner ? [] : 'var byteCount = 0;', inner ? [] : this._byteLengthCtx, inner ? [] : 'wBuffer._ensureWriteSize(byteCount);', inner ? [] : 'var buffer = wBuffer._buf;', this._encodeCtx, encodeSuffixs));
    }

    _buildByteLengthFuncStr(inner) {

        var prefixs = ['var strEnc = \'' + this._encoding + '\';', 'var byteCount = 0;'];
...
function getIterStr() {
    var iter = 'j' + _iterIndex;
    _iterIndex++;
    return iter;
}

function getArrayValStr() {
    var val = 'arrLen' + _arrayValIndex;
    _arrayValIndex++;
    return val;
}
...
function getBuiltinWriteString(dataType, valueStr) {
    switch (dataType) {
        case 'boolean':
            return _genWriteStr('UInt8', valueStr + ' ? 1 : 0');
        case 'int8':
        ...
        case 'int64be':
            return 'helper.writeInt64BE(buffer, ' + valueStr + ');';
        case 'int64le':
            return 'helper.writeInt64LE(buffer, ' + valueStr + ');';
        ...
        case 'uint64be':
            return 'helper.writeUInt64BE(buffer, ' + valueStr + ');';
        case 'uint64le':
            return 'helper.writeUInt64LE(buffer, ' + valueStr + ');';
        ...
        case 'varuint':
            return 'helper.writeVarUInt(buffer, ' + valueStr + ');';
        case 'varint':
            return 'helper.writeVarInt(buffer, ' + valueStr + ');';

        case 'string':
            return 'helper.writeString(buffer, ' + valueStr + ', strEnc);';
        case 'buffer':
            return 'helper.writeBuffer(buffer, ' + valueStr + ');';
    }
    return undefined;
}
...
function getBuiltinSizeString(dataType, valueStr) {
    ...
    if (!funcMap || !funcMap.size) return '';
    if (typeof funcMap.size === 'number') return 'byteCount += ' + funcMap.size + ';';
